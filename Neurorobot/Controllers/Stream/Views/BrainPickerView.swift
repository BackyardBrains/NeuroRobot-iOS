...
private extension BrainPickerView {
    @objc func doneButtonTapped() {
        guard let availablePaths = Brain.availableBrainPaths(), !availablePaths.isEmpty else { return }
        guard let availableBrains = Brain.availableBrains(), !availableBrains.isEmpty else { return }
        
        choosenBrainWithPath?(availablePaths[selectedRow], availableBrains[selectedRow])
    }
...
extension BrainPickerView: UIPickerViewDelegate, UIPickerViewDataSource {
    ...
    func pickerView(_ pickerView: UIPickerView, numberOfRowsInComponent component: Int) -> Int {
        return Brain.availableBrains()?.count ?? 0
    }
